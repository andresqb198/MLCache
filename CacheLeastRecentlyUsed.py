...
class Cache:
    ...
    def access_data(self, data):
        if data in self.cache:
            print("Hit!")
            self.hits += 1
            self.cache.remove(data)  # Eliminar el dato de la posición actual
            self.cache.append(data)  # Agregarlo al final de la cache (el más reciente)
        else:
            if len(self.cache) >= self.capacity:
                self.cache.pop(0)  # Eliminar el dato más antiguo (el menos reciente)
